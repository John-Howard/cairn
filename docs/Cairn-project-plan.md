...
flowchart LR
    ...
    P4["<b>Phase 4</b><br/>Data Migration<br/>& Pilot"]
...
| 4 | Data migration & pilot | Seeded vocabularies + migrated ROPA, piloted |
...
| 2d Hybrid workflow | Roles/permissions, questionnaire ingestion, propose-and-approve reference data, review cycles, trial lifecycle gates |
...
## 7. Phase 4 — Data Migration & Pilot

**Objective:** load real content and validate Cairn with real users on a limited scope.

**Key activities:** seed the FRS vocabularies; migrate the existing ROPA from current spreadsheet/templates (e.g. the controller/processor documentation templates) into the model, resolving the granular links Cairn requires; pilot with one or two business functions (e.g. Prevention and HR); refine vocabularies, workflow and rules from pilot feedback.

**Deliverables:** seeded reference data, migrated and linked ROPA, pilot findings and adjustments.

**Dependency:** Phase 3 (a trustworthy system to put real data into).
...
4. **Migration source** — confirm the current ROPA's format and quality (existing spreadsheets/templates) as the migration input for Phase 4.
5. **Pilot scope** — which one or two business functions pilot first.
