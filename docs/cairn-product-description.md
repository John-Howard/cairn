...
- **Built-in information audit** — a guided, jargon-free intake wizard runs the organisation-wide data-mapping exercise inside Cairn itself, with two question sets: one for the processing activities a team carries out, the other for the information assets it holds. Departments answer plain-English questions, each response becomes a draft record with its links already in place, unknown answers are captured as explicit gaps for follow-up, and the IG team works through them in an audited resolution queue. No spreadsheet round-trip, no separate survey tool.
...
- **Dashboard & KPIs, shaped by role** — the governance picture for the people accountable for it: register health, records due for review, asset coverage (owners assigned, reviews overdue, undocumented personal-data assets) and outstanding rule findings, with assurance measures and the commencement watch for the DPO. A curator lands on their work queues instead; a departmental contributor sees only their own intake and what to describe next, not a compliance dashboard they have no use for.
- **Navigation grouped by purpose** — one menu on every screen, separating the registers you read from the ways information is added, the curation queues, and administration. Each group is filtered to the signed-in role, so people are never shown a door they cannot open.
