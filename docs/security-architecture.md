...
- Login, logout, and failed/denied authorisations are recorded as `AuditEvent` rows — the same table that audits record changes. **Implemented (2026-07):** `login_succeeded` (OIDC), `logout`, and `authorisation_denied` (in-app 403s with path and roles) are audited; OIDC denials for unknown identities, which have no user row, go to the structured security log (subject only).
