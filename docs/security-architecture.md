...
- **Server-side sessions** after login: cookie is `Secure`, `HttpOnly`, `SameSite=Lax`; absolute lifetime 12h, idle timeout 60m (configurable per org). **Implemented (2026-07):** idle timeout via the rolling signed-cookie `max_age` (`SESSION_IDLE_SECONDS`, default 3600); absolute lifetime via an `auth_at` stamp set at login and checked on every authenticated request (`SESSION_ABSOLUTE_SECONDS`, default 43200). Sessions predating enforcement are treated as expired.
...
- Login, logout, and failed/denied authorisations are recorded as `AuditEvent` rows — the same table that audits record changes. **Implemented (2026-07):** `login_succeeded` (OIDC), `logout`, and `authorisation_denied` (in-app 403s with path and roles) are audited; OIDC denials for unknown identities, which have no user row, go to the structured security log (subject only). Logout is **RP-initiated** in oidc mode — the IdP session is ended too (end-session redirect with logout hint), with graceful local-only fallback if the IdP is unreachable.
